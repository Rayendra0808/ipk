...
class DimensionTotalValueProvincesTable2Seeder extends Seeder
{
  /**
  ...
  public function run()
  {
    ...
    $csvFile = fopen(base_path("database/data/dimension_total_value_provinces_2022.csv"), "r");
    ...
    while (($data = fgetcsv($csvFile, 200, ",")) !== FALSE) {
      if (!$firstline) {
        DimensionTotalValueProvince::create([
          ...
          "total" => $data[1],
          "year" => $data[2],
          "desc" => isset($data[3]) && $data[3] != '' ? $data[3] : null,
        ]);
      }
      $firstline = false;
    }
